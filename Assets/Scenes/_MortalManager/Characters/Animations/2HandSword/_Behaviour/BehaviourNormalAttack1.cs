...
public class BehaviourNormalAttack1 : StateMachineBehaviour
{
    [Header("Filter")]
    [SerializeField] private string onlyStateName = "NormalAttack_1";

    [Header("Timings (normalized 0~1)")]
    [Range(0f, 1f)] public float slashStartN = 0.05f;  // 트레일/스윙 시작
    ...
    public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        animator.GetComponentInParent<MyCharacterCore>()?.OnNormalAttackEnter(1);
    }
    ...
    public override void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        {
            _firedStart = true;
            core.OnSwordSlashStart(1);
        }

        if (!_firedHit && n >= hitN)
        {
            _firedHit = true;
            core.OnSwordSlashHit(1);
        }

        if (!_firedEnd && n >= slashEndN)
        {
            _firedEnd = true;
            core.OnSwordSlashEnd(1);
        }
    }
    ...
    public override void OnStateExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        animator.GetComponentInParent<MyCharacterCore>()?.OnNormalAttackExit(1);
    }
